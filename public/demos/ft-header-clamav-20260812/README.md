...
```bash
unzip ft-header-clamav-20260812.zip
cd ft-header-clamav-20260812
```
...
- Open `malicious-doc.docx` in its native app — the attack triggers on open.
...
- Open `clean-doc.docx` — no payload, no calculator.
...
- [ ] `unzip -t ft-header-clamav-20260812.zip` → no errors

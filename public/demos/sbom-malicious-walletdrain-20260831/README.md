...
The 2026-08-29 CISO Daily Digest reported that 19 Chrome and Edge browser extensions were found carrying wallet-stealing and crypto-draining code, silently exfiltrating cryptocurrency from affected users — a supply-chain-driven campaign. The same digest flagged active exploitation of the Cosmos EVM flaw (Cosmos Labs) and chained PaperCut flaws (CVE-2023-27350 / CVE-2023-27351, unauthenticated remote code execution). Like the vast majority of modern web software, these extensions are assembled from third-party npm packages, and a single compromised or typosquatted dependency is all it takes to inject wallet-draining logic into a shipped product. This demo reproduces the supply-chain root safely: a synthetic `malicious-package.json` stands in for an extension's build manifest that pulls a typosquatted dependency (`walletconnect-utils`, mimicking the legitimate `walletconnect` library) engineered to siphon private keys, while the `clean-package.json` counterpart pins only vetted, integrity-checked components. Nothing is executed and no real keys are touched; the only effect is to show how a malicious dependency enters the build. OPSWAT SBOM analysis inspects the dependency tree BEFORE the artifact ships, identifies the malicious / typosquatted component, and blocks it — so a wallet-draining extension never reaches end users (MITRE ATT&CK T1195.001 Supply Chain Compromise: Software Dependencies and Development Tools).
...
Compromise of software dependencies — a typosquatted / malicious npm package (walletconnect-utils mimicking walletconnect) bundled into an extension's build to drain crypto wallets (T1195.001 Supply Chain Compromise: Software Dependencies and Development Tools) (MITRE ATT&CK [T1195.001](https://attack.mitre.org/techniques/T1195.001/))

...
本週多個攻擊行動以**檔案**為載體：PATCHCORD 的假 VPN 安裝程式、Mustang Panda 的已簽章核心驅動程式、WindRelay 的側載 APK，以及透過假冒下載頁散播的 macOS 竊密程式。MetaDefender 的多重掃描與**內容無害化（CDR）**能在惡意套件、安裝程式與文件到達端點前將其淨化——包括傳統防毒視為可信的「已簽章但惡意」二進位檔。

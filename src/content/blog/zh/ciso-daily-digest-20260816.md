...
WordPress 發布 **7.0.3** 安全更新，修補核心登入頁面的高風險跨站指令碼（XSS）漏洞 **CVE-2026-64638（XSS2Shell）**。漏洞位於登入流程：當訪客以不存在的帳號名稱嘗試登入時，WordPress 會將輸入內容帶入錯誤訊息，而前後兩道 HTML 過濾機制對異常標記的判讀方式不一致，讓原本應被移除的惡意內容殘留在登入頁面並被瀏覽器解析成有效 HTML 元素。攻擊者**不需要先取得 WordPress 帳號**即可觸發漏洞。此漏洞於 8 月中旬首次公布，本欄 8 月 13 日已報導相關揭露——本週新增的關鍵是攻擊規模。
